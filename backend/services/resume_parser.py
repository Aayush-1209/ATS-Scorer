...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    # 1. Try using python-magic if it loaded successfully
    ...
    # 2. Fallback to file signature (magic bytes) checks
    if not mime_type:
        if file_data.startswith(b'%PDF'):
            mime_type = 'application/pdf'
        elif file_data.startswith(b'PK\x03\x04'):
            # Docx is a zip archive; check file extension to be precise
    ...
    # 3. Fallback to standard library mimetype guess based on filename
    if not mime_type:
    ...
    if not mime_type or mime_type not in SUPPORTED_MIME_TYPES:
        supported_names = ', '.join(set(SUPPORTED_MIME_TYPES.values())).upper()
        return False, (
            f'Unsupported or invalid file type: {mime_type or "unknown"}. '
            f'Please upload a valid file of type: {supported_names}.'
        ), None
